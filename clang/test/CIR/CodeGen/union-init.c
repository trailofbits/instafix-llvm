...
// CHECK-DAG: !rec_{{[A-Za-z0-9_]+}} = !cir.record<struct "A::anon" {!u32i}{{.*}}>
// CHECK-DAG: !rec_U = !cir.record<union "U" {!s32i}{{.*}}>
...
// CHECK-DAG: !rec_{{[A-Za-z0-9_.E]+}} = !cir.record<union "{{[A-Za-z0-9_.]+}}" {!u32i, !cir.array<!u8i x 4>}{{.*}}>
...
// CHECK:  %[[VAL_3:.*]] = cir.get_member %[[VAL_2]][1] {name = ""} : !cir.ptr<!rec_A> -> !cir.ptr<!rec_{{[A-Za-z0-9_]+}}>
// CHECK:  %[[VAL_4:.*]] = cir.get_member %[[VAL_3]][0] {name = "x"} : !cir.ptr<!rec_{{[A-Za-z0-9_]+}}> -> !cir.ptr<!u32i>
...
// CHECK:  %[[VAL_7:.*]] = cir.get_member %[[VAL_3]][0] {name = "y"} : !cir.ptr<!rec_{{[A-Za-z0-9_]+}}> -> !cir.ptr<!u32i>
// CHECK:  %[[VAL_8:.*]] = cir.const #cir.int<0> : !s32i
// CHECK:  %[[VAL_9:.*]] = cir.set_bitfield align(4) (#bfi_y, %[[VAL_7]] : !cir.ptr<!u32i>, %[[VAL_8]] : !s32i) -> !s32i
// CHECK:  cir.return

union { int i; float f; } u = { };
// CHECK: cir.global external @u = #cir.zero : !rec_{{[A-Za-z0-9_.E]+}}
...
// CHECK: %[[VAL_1:.*]] = cir.get_global @is_little.one : !cir.ptr<!rec_anon_struct>
// CHECK: %[[VAL_2:.*]] = cir.cast(bitcast, %[[VAL_1]] : !cir.ptr<!rec_anon_struct>), !cir.ptr<!rec_{{[A-Za-z0-9_.E]+}}>
// CHECK: %[[VAL_3:.*]] = cir.get_member %[[VAL_2]][1] {name = "c"} : !cir.ptr<!rec_{{[A-Za-z0-9_.E]+}}> -> !cir.ptr<!cir.array<!u8i x 4>>
